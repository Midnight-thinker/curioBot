...
const QuizResultPage = () => {
  ...
  const getScoreColor = (score) => {
    ...
    if (score >= 80) return "from-violet-500 to-teal-500"
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr,1fr]">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-violet-600/80">Score</p>
        ...
        <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50/80 p-6 shadow-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between text-sm text-slate-700">
              ...
              <span className="font-semibold text-violet-700">{correctAnswers}</span>
            </div>
            <div className="flex items-center justify-between text-sm text-slate-700">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="inline-flex items-center gap-2 rounded-full bg-violet-100/80 px-3 py-1 text-xs font-semibold text-violet-800">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-600">
                    Your answer: <span className="font-semibold text-slate-900">{item?.userAnswer || "No answer"}</span>
                  </p>
                  <p className="text-sm text-slate-600">
                    Correct answer: <span className="font-semibold text-violet-700">{item?.correctAnswer || "N/A"}</span>
                  </p>
                </div>
                <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${item?.isCorrect ? "bg-violet-100/80 text-violet-800" : "bg-rose-100/80 text-rose-800"}`}>
                  {item?.isCorrect ? (
                    <>
